...
const COVER_COUNT=ARTICLES.length;
function formattedCoverCount(value=COVER_COUNT){return value<100?String(value).padStart(2,'0'):String(value)}
function coverCountMarkup(){return `<span class="cover-count" aria-label="${COVER_COUNT} copertine"><span class="cover-count-bracket">[</span><span class="cover-count-number">${formattedCoverCount()}</span><span class="cover-count-bracket">]</span></span>`}
function updateCoverCountLabels(){
 document.querySelectorAll('header nav a[href$="copertine.html"], .world-covers').forEach(link=>{
  const label=link.classList.contains('world-covers')?'Le copertine':'Copertine';
  link.innerHTML=`${label} ${coverCountMarkup()}`;
 });
 document.querySelectorAll('[data-cover-count-title]').forEach(title=>{title.innerHTML=`<span class="covers-title-text">Le Copertine</span>${coverCountMarkup()}`});
}
function animateCoverCount(){
 const number=document.querySelector('[data-cover-count-title] .cover-count-number');
 if(!number||window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;
 const duration=1400;
 number.textContent=formattedCoverCount(0);
 setTimeout(()=>{
  const start=performance.now();
  function frame(now){
   const progress=Math.min(1,(now-start)/duration);
   const eased=1-Math.pow(1-progress,3);
   number.textContent=formattedCoverCount(Math.round(COVER_COUNT*eased));
   if(progress<1)requestAnimationFrame(frame);
  }
  requestAnimationFrame(frame);
 },200);
}
updateCoverCountLabels();
animateCoverCount();
const FLASH_ITEMS=[
 ...
 'Dal 2 agosto 2026 entrano in applicazione nell’Unione europea gli obblighi di trasparenza sui contenuti sintetici'
];
const SOURCES={
...
const PHOTO_COVERS={
 ...
 'contenuti-sintetici-regole-trasparenza':{
  ...
  alt:'Una figura attraversa un archivio monumentale di fotografie e memorie'
 },
 'ai-puo-avere-intuizione':{
...
function editorialDate(iso){return new Intl.DateTimeFormat('it-IT',{day:'numeric',month:'long',year:'numeric',timeZone:'Europe/Rome'}).format(new Date(`${iso}T12:00:00Z`))}
...
const FAVORITES_KEY='onoff-visions-cover-favorites-v1';
function readFavorites(){try{return new Set(JSON.parse(localStorage.getItem(FAVORITES_KEY)||'[]'))}catch(error){return new Set()}}
function writeFavorites(favorites){try{localStorage.setItem(FAVORITES_KEY,JSON.stringify([...favorites]))}catch(error){}}
function favoriteButton(a,extraClass=''){const active=readFavorites().has(a.slug);return `<button class="cover-favorite ${extraClass} ${active?'is-favorite':''}" type="button" data-favorite-cover="${a.slug}" aria-pressed="${active}" aria-label="${active?'Rimuovi':'Aggiungi'} ${a.title} dalle copertine preferite" title="${active?'Rimuovi dalle preferite':'Aggiungi alle preferite'}"><span aria-hidden="true">${active?'♥':'♡'}</span></button>`}
function bindFavoriteButtons(root=document){root.querySelectorAll('[data-favorite-cover]').forEach(button=>{if(button.dataset.favoriteBound)return;button.dataset.favoriteBound='true';button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const favorites=readFavorites();const slug=button.dataset.favoriteCover;const articleData=ARTICLES.find(item=>item.slug===slug);if(favorites.has(slug))favorites.delete(slug);else favorites.add(slug);writeFavorites(favorites);const active=favorites.has(slug);document.querySelectorAll(`[data-favorite-cover="${slug}"]`).forEach(item=>{item.classList.toggle('is-favorite',active);item.setAttribute('aria-pressed',String(active));item.setAttribute('title',active?'Rimuovi dalle preferite':'Aggiungi alle preferite');item.setAttribute('aria-label',`${active?'Rimuovi':'Aggiungi'} ${articleData?articleData.title:'questa copertina'} dalle copertine preferite`);const symbol=item.querySelector('span');if(symbol)symbol.textContent=active?'♥':'♡'})})})}
const coverGrid=document.querySelector('[data-cover-grid]');if(coverGrid){const covers=ARTICLES.filter(a=>PHOTO_COVERS[a.slug]);coverGrid.innerHTML=covers.map(a=>{const photo=PHOTO_COVERS[a.slug];return `<article class="cover-tile"><a class="cover-tile-link" href="${base}/articoli/${a.slug}.html" aria-label="${a.n}: ${a.title}"><img src="${base}/${photo.small}" alt="${photo.alt}" loading="lazy" decoding="async"><span><b>${a.n}</b><strong>${highlightedTitle(a)}</strong></span></a>${favoriteButton(a)}</article>`}).join('');bindFavoriteButtons(coverGrid);document.querySelectorAll('[data-cover-mode]').forEach(button=>button.addEventListener('click',()=>{const showTitles=button.dataset.coverMode==='titles';coverGrid.classList.toggle('show-titles',showTitles);document.querySelectorAll('[data-cover-mode]').forEach(item=>{const active=item===button;item.classList.toggle('is-active',active);item.setAttribute('aria-pressed',String(active))})}))}
...
if(article){
 ...
 if(a){
  ...
  const photoFeature=photo?`<figure class="article-cover-photo"><picture><source media="(max-width:800px)" srcset="${base}/${photo.small}"><img src="${base}/${photo.large}" alt="${photo.alt}" decoding="async"></picture><figcaption><strong>${photo.phrase}</strong><small>Immagine elaborata dall’archivio fotografico di Claudio Napoli</small></figcaption>${favoriteButton(a,'article-cover-favorite')}</figure>`:'';
  const updatedMeta=a.updated&&a.updated!==a.iso?` · Aggiornato il ${editorialDate(a.updated)}`:'';
  const hero=`<section class="article-hero"><div class="article-number">${numberMark(a.n)}</div><div class="article-intro"><h1>${a.title}</h1><div class="article-editorial-meta"><span>ONOFF VISIONS</span><span aria-hidden="true">·</span><span>${a.n}</span><span aria-hidden="true">·</span><time datetime="${a.iso}">${editorialDate(a.iso)}</time>${updatedMeta}${isNew(a)?'<span class="new">NEW</span>':''}</div><p class="deck">${a.deck}</p><div class="meta article-reading-meta"><span>${displayTime(a)} di lettura</span></div><button class="listen-button" type="button" data-listen aria-pressed="false"><span class="listen-symbol">▶</span><span data-listen-label>Ascolta l’articolo</span></button></div></section>`;
  if(a===PROJECT){
   ...
   article.innerHTML=`${hero}${photoFeature}<div class="story-wrap"><section class="story-block story-essential"><aside><b>01</b><span>L’essenziale</span></aside><div class="story-copy">${sections.essential.map(p=>`<p>${p}</p>`).join('')}</div></section><section class="story-block story-deep"><aside><b>02</b><span>L’approfondimento</span></aside><div class="story-copy">${sections.deep.map(p=>`<p>${p}</p>`).join('')}</div></section><section class="story-block story-summary"><aside><b>03</b><span>In due righe</span></aside><div class="story-copy"><p>${sections.summary}</p><div class="highlights"><span class="eyebrow" style="width:100%">Parole chiave</span>${a.tags.map(t=>`<span class="chip">${t}</span>`).join('')}</div></div></section><section class="story-block story-sources"><aside><b>04</b><span>Per approfondire</span></aside><div class="story-source-list">${sources}</div></section><section class="article-project-signature"><p><strong>Questo è l’articolo ${a.n.split('/')[0]} del progetto ONOFF VISIONS 365.</strong></p><p><em>Un percorso editoriale che racconta come l’innovazione sta trasformando il nostro modo di creare, comunicare e immaginare il futuro.</em></p><div>ONOFF VISIONS — Un articolo al giorno per raccontare il mondo che cambia.</div></section><a class="archive-link story-home" href="../index.html"><span>← Torna alla homepage</span></a></div>`;
  }
